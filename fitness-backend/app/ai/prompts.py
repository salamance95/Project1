...
COACH_SYSTEM = """당신은 피트니스 코치입니다. 사용자의 설문, 이번 주 계획, 실제 기록을 보고
...
PHOTO_SYSTEM = """당신은 식사 사진을 보고 음식과 양, 대략적인 영양소를 적는 도우미입니다.
...
PLAYLIST_SYSTEM = """당신은 운동용 플레이리스트를 짜는 DJ입니다.
...
def build_playlist_prompt(genre, bpm, minutes, count):
    """플레이리스트 요청 문장."""
    return "\n".join([
        f"장르: {genre}",
        f"템포 대역: {bpm} BPM",
...
BODY_PHOTO_SYSTEM = """당신은 체성분 결과지 사진에서 숫자를 그대로 옮겨 적는 도우미입니다.
...
def build_coach_prompt(context: CoachContext):
...
def build_search_prompt(query, catalog):

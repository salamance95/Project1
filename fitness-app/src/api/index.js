...
/**
 * 오류 본문을 사람이 읽을 문장으로 바꾼다.
 * 검증 실패(422)는 detail이 항목별 객체 배열로 와서, 그냥 쓰면 [object Object]가 찍힌다.
 */
function describeDetail(detail) {
  ...
  if (Array.isArray(detail)) {
    const lines = detail.map((item) => {
      ...
      return field ? `${field}: ${item.msg ?? "잘못된 값"}` : (item.msg ?? "잘못된 값");
    });
    return `입력값을 확인해 주세요 — ${lines.join(", ")}`;
  }

  return detail.msg ?? "요청을 처리하지 못했습니다.";
}
...
async function request(path, { method = "GET", body, signal } = {}) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (payload.detail) detail = describeDetail(payload.detail);
    } catch {
...
export const api = {
  ...
  linkSupabaseUser: (payload, signal) =>
    request("/api/auth/supabase/link", { method: "POST", body: payload, signal }),

  authConfig: (signal) => request("/api/auth/config", { signal }),

  exerciseHistory: (userId, signal) =>
    request(`/api/logs/exercise-history?user_id=${userId}`, { signal }),
  ...
  analyzeMealPhoto: async (file) => {
    ...
    // FormData는 Content-Type을 브라우저가 직접 정해야 해서 request()를 쓰지 않는다.
    const response = await fetch(`${API_BASE_URL}/api/nutrition/photo`, {
      method: "POST",
      body: form,
    });

    if (!response.ok) {
      let detail = `사진 분석에 실패했습니다 (${response.status})`;
      try {
        const payload = await response.json();
  ...
  analyzeBodyPhoto: async (file) => {
    const form = new FormData();
    form.append("photo", file);

    // FormData는 Content-Type을 브라우저가 직접 정해야 해서 request()를 쓰지 않는다.
    const response = await fetch(`${API_BASE_URL}/api/body/photo`, {
      method: "POST",
      body: form,
    });
  ...
  exerciseAlternatives: (slug, userId, signal) =>
    request(`/api/exercises/${slug}/alternatives?user_id=${userId ?? ""}`, { signal }),

  swapExercise: (payload) =>
    request("/api/plans/swap-exercise", { method: "POST", body: payload }),

  music: (userId, date, signal) =>
    request(`/api/music?user_id=${userId ?? ""}&date=${date ?? ""}`, { signal }),

  musicTracks: (theme, duration, signal) =>
    request(`/api/music/tracks?theme=${theme}&duration=${duration}`, { signal }),

  addMusicLink: (payload) => request("/api/music/links", { method: "POST", body: payload }),

  deleteMusicLink: (userId, linkId) =>
    request(`/api/music/links/${linkId}?user_id=${userId}`, { method: "DELETE" }),
